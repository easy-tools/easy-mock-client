...
class EasyMockClient {//  extends EasyMockClientAbstract
  ...
  deleteMock(mockId) {
    if (typeof mockId === 'undefined' || !mockId) {
      return Promise.reject('参数错误')
    }

    const handler = () => {
      return this.getRequest().post('/api/mock/delete', Object.assign({
        id: mockId
      })).then(response => {
        const responseData = response.data
        if (!responseData.success) {
  ...
  createProject(projectData) {
    ...
    const handler = () => {
      return this.getRequest().post('/api/project/create', Object.assign({
  ...
  deleteProject(projectId) {
    ...
    const handler = () => {
      return this.getRequest().post('/api/project/delete', Object.assign({
        id: projectId,
      })).then(response => {
        const responseData = response.data
        if (!responseData.success) {
